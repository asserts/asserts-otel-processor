...
func (s *sampler) updateTrace(namespace string, service string, trace *traceStruct) {
	...
	s.logger.Debug("Trace summary",
		zap.String("traceId", trace.rootSpan.TraceID().String()),
		zap.String("request", trace.requestKey.AsString()),
		zap.Bool("isSlow", trace.isSlow),
		zap.Bool("error", trace.hasError()),
		zap.Float64("latency", trace.latency))
}
