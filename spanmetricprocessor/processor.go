...
type processorImp struct {
	...
	metricKeyToDimensions *Cache[metricKey, pcommon.Map]
...
func newProcessor(logger *zap.Logger, config component.Config, nextConsumer consumer.Traces, ticker *clock.Ticker) (*processorImp, error) {
	logger.Info("Building assertsotelprocessor")
	...
	metricKeyToDimensionsCache, err := NewCache[metricKey, pcommon.Map](pConfig.DimensionsCacheSize)
	if err != nil {
		return nil, err
	}
...
func (p *processorImp) Start(ctx context.Context, host component.Host) error {
	p.logger.Info("Starting assertsotelprocessor")
	...
	for k, exp := range exporters[component.DataTypeMetrics] {
		...
		p.logger.Debug("Looking for assertsotelprocessor from available exporters",
			zap.String("assertsotelprocessor", p.config.MetricsExporter),
			zap.Any("available-exporters", availableMetricsExporters),
		)
		if k.String() == p.config.MetricsExporter {
			p.metricsExporter = metricsExp
			p.logger.Info("Found exporter", zap.String("asserts-otel-exporter", p.config.MetricsExporter))
			break
		}
	}
	...
	p.logger.Info("Started assertsotelprocessor")
...
func (p *processorImp) Shutdown(context.Context) error {
	p.shutdownOnce.Do(func() {
		p.logger.Info("Shutting down assertsotelprocessor")
		if p.started {
...
func (p *processorImp) buildMetrics() pmetric.Metrics {
	...
	ilm.Scope().SetName("assertsotelprocessor")
	...
	for i := 0; i < traces.ResourceSpans().Len(); i++ {
		...
		for j := 0; j < ilsSlice.Len(); j++ {
			...
			for k := 0; k < spans.Len(); k++ {
				span := spans.At(k)
				// We only care about AWS api spans
				rpcSystem, present := span.Attributes().Get("rpc.system")
				if present && rpcSystem.Str() == "aws-api" {
