...
func TestProcessorStart(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create assertsotelprocessor processor
			factory := NewFactory()
...
func TestProcessorConcurrentShutdown(t *testing.T) {
	...
	// Starting assertsotelprocessor...
	// Started assertsotelprocessor...
	// Shutting down assertsotelprocessor...
	// Stopping ticker.
...
func verifyConsumeMetricsInput(t testing.TB, input pmetric.Metrics, expectedTemporality pmetric.AggregationTemporality, numCumulativeConsumptions int) bool {
	...
	assert.Equal(t, "assertsotelprocessor", ilm.At(0).Scope().Name())
