...
The following is a simple example usage of the assertsotelprocessor processor.
...
processors:
  batch:
  assertsotelprocessor:
    metrics_exporter: otlp/assertsotelprocessor
...
service:
  pipelines:
    traces:
      receivers: [jaeger]
      processors: [assertsotelprocessor, batch]
      exporters: [jaeger]

    # The exporter name must match the metrics_exporter name.
    # The receiver is just a dummy and never used; added to pass validation requiring at least one receiver in a pipeline.
    metrics/assertsotelprocessor:
      receivers: [otlp/assertsotelprocessor]
      exporters: [otlp/assertsotelprocessor]
